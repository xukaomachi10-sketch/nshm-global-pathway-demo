    ...
    <div className="space-y-6 animate-rise">
        ...
        {[
          ...
          <Card key={String(l)} className="p-4">
            <div className="flex items-center gap-3">
              <div
                className={`rounded-xl p-2.5 text-white ${c === "red" ? "bg-[#D21235]" : c === "gold" ? "bg-[#FFAD00]" : c === "green" ? "bg-[#2DA037]" : "bg-[#23328C]"}`}
              >
                <I className="h-4 w-4" />
              </div>
              <div>
                ...
                <p className="text-xl font-black text-[#23328C]">{String(v)}</p>
              </div>
            </div>
            {i === 4 ? (
              <div className="mt-3">
                  ...
                  {rows.map((d, i) => (
                    <tr key={d.name} className="hover:bg-slate-50">
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-3">
                          <div
                            className={`rounded-xl p-2.5 ${d.category === "Essays" ? "bg-[#FAC7D0]/35 text-[#D21235]" : "bg-[#ADDDFF]/35 text-[#23328C]"}`}
                          >
                            {d.category === "Essays" ? (
                          ...
                          <div>
                            <button
                              ...
                              className="font-black text-[#23328C] hover:text-[#D21235]"
                            >
                              {d.name}
                            </button>
                      ...
                      <td className="px-4 py-4">
                        ...
                        {i < 2 && (
                          <p className="text-xs text-[#D21235]">Sắp đến hạn</p>
                        )}
                      </td>
                      <td className="px-5 py-4">
                        <div className="flex gap-1">
                          <button
                            onClick={() => setSelected(d)}
                            title="Xem chi tiết"
                            className="rounded-lg p-2 text-slate-400 hover:bg-slate-100 hover:text-[#D21235]"
                          >
        ...
        <div className="min-w-0 space-y-4">
          <Card className="p-5">
            <div className="flex items-center justify-between">
              ...
              <span className="text-2xl font-black text-[#D21235]">76%</span>
            </div>
            <div className="mt-4">
              <Progress value={76} />
            </div>
              ...
              {[
                ...
                <div key={String(x)}>
                  <div className="mb-1.5 flex justify-between text-xs">
                    ...
                    <span
                      className={
                        a === b ? "text-[#2DA037]" : "text-slate-400"
                      }
                    >
                      {a}/{b}
                    </span>
          ...
          <Card className="p-5">
            <div className="flex items-center gap-2">
              <AlertTriangle className="h-5 w-5 text-[#D21235]" />
              <h3 className="font-black text-[#23328C]">Cảnh báo tài liệu</h3>
            </div>
            <div className="mt-4 space-y-3">
              {[
                ...
                >
                  <span
                    className={`h-2 w-2 rounded-full ${x[2] === "red" ? "bg-[#D21235]" : x[2] === "amber" ? "bg-[#FFAD00]" : "bg-[#23328C]"}`}
                  />
                  <div>
          ...
          <Card className="p-5">
            <div className="flex items-center justify-between">
              <h3 className="font-black text-[#23328C]">Phản hồi gần nhất</h3>
              <MessageSquare className="h-4 w-4 text-slate-400" />
            </div>
            <div className="mt-4 rounded-xl border-l-4 border-[#D21235] bg-[#FAC7D0]/35 p-4">
              <p className="text-sm leading-6 text-slate-700">
                “Phần mở bài đã có hình ảnh tốt. Em cần làm rõ khoảnh khắc thay
                đổi nhận thức và giảm khoảng 60 từ ở đoạn 3.”
              </p>
              <p className="mt-2 text-xs font-bold text-[#D21235]">
      ...
      >
        <div className="grid gap-6 md:grid-cols-[1fr_1.1fr]">
          ...
          <div>
            ...
            <h3 className="mt-5 text-sm font-black text-[#23328C]">
              Lịch sử phiên bản
            </h3>
            <div className="mt-3 space-y-1">
              {versions.map((v, i) => (
                <div
                  key={v.v}
                  className={`relative flex gap-3 rounded-xl p-3 ${i === 0 ? "bg-[#FAC7D0]/35" : "hover:bg-slate-50"}`}
                >
                  <div
                    className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-xs font-black ${i === 0 ? "bg-[#D21235] text-white" : "bg-slate-100 text-slate-500"}`}
                  >
                    {v.v}
                  </div>
                  <div>
      ...
      >
        <div className="space-y-4">
          <button className="flex w-full flex-col items-center rounded-2xl border-2 border-dashed border-slate-300 bg-slate-50 py-10 hover:border-[#D21235]">
          ...
          <label className="flex items-start gap-3 rounded-xl bg-[#ADDDFF]/35 p-3">
            <input type="checkbox" defaultChecked className="mt-1" />
            <span className="text-xs leading-5 text-[#23328C]">
