...
create table if not exists public.student_intake_assessments (
  ...
  assigned_counselor_id uuid not null references public.staff_profiles(id) on delete restrict,
  ...
  deadline_risk_level public.risk_level,
  ...
  confidentiality_level text default 'D2' check (
